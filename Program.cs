...
public class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        .File("WorkerEnrollmentManagmente.out", Serilog.Events.LogEventLevel.Debug, "{Message:lj}{NewLine}", encoding: Encoding.UTF8)
        .CreateLogger();
    ...
    private static async void ConsumerReceived(object? sender, BasicDeliverEventArgs e)
    {
        ...
        EnrollmentResponse eResponse = await ClientWebServices(request);
        if (eResponse != null && eResponse.Codigo != 201)
        {
            channel.BasicPublish("", "kalum.queue.failed-enrollment", null, Encoding.UTF8.GetBytes(message));
        }
        else
        {
            AppLog.Message = "Se proceso la inscripción de manera exitosa";
    ...
    public static async Task<EnrollmentResponse> ClientWebServices(EnrollmentRequest request)
    {
        EnrollmentResponse eResponse = null;
        try
        {
            ...
            eResponse = new EnrollmentResponse()
            {
            ...
            if (eResponse.Codigo == 503)
            {
                ImprimirLog(503, eResponse.Respuesta, "Error");
            }
            else
            {
                ImprimirLog(201, $"Se finalizó el proceso de inscripción con el carné {eResponse.Carne}", "Information");
            }
        }
        catch (Exception e)
        {
            eResponse = new EnrollmentResponse() { Codigo = 500, Respuesta = e.Message, Carne = "0" };
